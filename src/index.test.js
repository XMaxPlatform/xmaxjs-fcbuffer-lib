...
describe('API', function () {
  ...
  it('string', function () {
    const {string} = Types()
    const type = string()
    assertSerializer(type, 'asdf123你们好')
    assertRequired(type)
  })
...
describe('Override', function () {
  ...
  it('type', function () {
    ...
    const asset = structs.asset.fromObject('1 XMX')
    assert.deepEqual(asset, {amount: 1, symbol: 'XMX'})
    assert.deepEqual('1 XMX', structs.asset.toObject(asset))
  })
...
describe('Custom Type', function () {
  it('Implied Decimal', function () {
    ...
    assert.equal(errors.length, 0)
    const asset = structs.asset.fromObject({amount: '1', symbol: 'XMX'})
    assert.deepEqual(asset, {amount: '1.0000', symbol: 'XMX'})
  })
})
