...
function fromBuffer(type, buffer) {
  ...
  var buf = ByteBuffer.fromBinary(buffer.toString('binary'), ByteBuffer.LITTLE_ENDIAN);
  var struct = type.fromByteBuffer(buf);
  return toObject ? type.toObject(struct) : struct;
}

function toByteBuffer(type, value) {
  var buf = new ByteBuffer(ByteBuffer.DEFAULT_CAPACITY, ByteBuffer.LITTLE_ENDIAN);
